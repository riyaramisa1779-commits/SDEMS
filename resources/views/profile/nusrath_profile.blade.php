...
<x-slot name="header">
    <div class="flex items-center justify-between">
        ...
        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-gradient-to-r from-rose-500 to-pink-600 text-white shadow-md ring-2 ring-rose-300 dark:ring-rose-700">
            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
            </svg>
            Senior Investigator Profile &mdash; Nusrath
        </span>
    </div>
</x-slot>
...
{{-- Hero Banner --}}
<div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-rose-500 via-pink-600 to-fuchsia-600 p-6 shadow-xl">
    ...
    <div class="relative flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        <div class="flex-1 min-w-0">
            ...
            <p class="text-rose-100 text-sm truncate">{{ $user->email }}</p>
            <p class="text-rose-200 text-xs mt-1">Dedicated profile &bull; Full access to evidence management &amp; custody transfers</p>
        </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="space-y-5">
        ...
        <div class="card p-6 text-center">
            <div class="relative inline-block mb-4">
                <div class="w-20 h-20 rounded-full bg-gradient-to-br from-rose-500 to-pink-600 flex items-center justify-center text-white font-bold text-3xl mx-auto shadow-lg">
                    {{ strtoupper(substr($user->name, 0, 1)) }}
                </div>
            ...
            <div class="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300 ring-1 ring-rose-200 dark:ring-rose-700">
                <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                </svg>
                Senior Investigator
        ...
        <div class="card p-5">
            ...
            <div class="space-y-2.5 text-sm">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="font-medium text-rose-600 dark:text-rose-400">{{ $user->rank }} / 10</span>
                </div>
            </div>
        </div>
    ...
    <div class="lg:col-span-2 space-y-5">
                ...
                <form method="POST" action="{{ route('profile.update') }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" value="Rank {{ $user->rank }} — Senior Investigator" disabled class="form-input bg-slate-50 dark:bg-slate-700/50 text-slate-500 cursor-not-allowed"/>
                        </div>
                        <div>
                            <label class="form-label">Role</label>
                            <input type="text" value="{{ $user->getRoleNames()->first() ?? 'No Role Assigned' }}" disabled class="form-input bg-slate-50 dark:bg-slate-700/50 text-slate-500 cursor-not-allowed"/>
                        </div>
                    </div>
        ...
        <div class="card"
             x-data="{
                 showCurrent: false, showNew: false, showConfirm: false, password: '',
                 get strength() { let s=0; if(this.password.length>=12)s++; if(/[A-Z]/.test(this.password))s++; if(/[0-9]/.test(this.password))s++; if(/[\W_]/.test(this.password))s++; return s; },
                 get strengthLabel() { return ['','Weak','Fair','Good','Strong'][this.strength]; },
                 get strengthColor() { return ['bg-slate-200 dark:bg-slate-600','bg-red-500','bg-amber-500','bg-yellow-400','bg-emerald-500'][this.strength]; },
                 get strengthTextColor() { return ['text-slate-400','text-red-500','text-amber-500','text-yellow-500','text-emerald-500'][this.strength]; }
             }">
            <div class="card-header">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('profile.password.update') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input :type="showNew ? 'text' : 'password'" name="password" x-model="password" required placeholder="Min 12 chars, mixed case, numbers, symbols" class="form-input pr-10 {{ $errors->has('password') ? 'form-input-error' : '' }}"/>
                        ...
                        <div class="mt-2 space-y-1" x-show="password.length > 0" x-transition>
                            <div class="flex gap-1">
                                <template x-for="i in 4" :key="i">
                                    <div class="h-1.5 flex-1 rounded-full transition-colors duration-300" :class="i <= strength ? strengthColor : 'bg-slate-200 dark:bg-slate-600'"></div>
                                </template>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
                                <span :class="password.length >= 12 ? 'text-emerald-600 dark:text-emerald-400' : ''"><span x-text="password.length >= 12 ? '✓' : '○'"></span> 12+ chars</span>
                                <span :class="/[A-Z]/.test(password) ? 'text-emerald-600 dark:text-emerald-400' : ''"><span x-text="/[A-Z]/.test(password) ? '✓' : '○'"></span> Uppercase</span>
                                <span :class="/[0-9]/.test(password) ? 'text-emerald-600 dark:text-emerald-400' : ''"><span x-text="/[0-9]/.test(password) ? '✓' : '○'"></span> Number</span>
                                <span :class="/[\W_]/.test(password) ? 'text-emerald-600 dark:text-emerald-400' : ''"><span x-text="/[\W_]/.test(password) ? '✓' : '○'"></span> Symbol</span>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input :type="showConfirm ? 'text' : 'password'" name="password_confirmation" required class="form-input pr-10"/>
        ...
        <div class="card" x-data="{ showDisable: false }">
            ...
            <div class="card-body">
                @if($user->hasTwoFactorEnabled())
                    ...
                    <div class="mb-4">
                        <p class="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-2">Recovery Codes &mdash; store these safely:</p>
                        <div class="bg-slate-900 dark:bg-slate-950 rounded-lg p-4 grid grid-cols-2 gap-1.5">
                            @foreach($recoveryCodes as $code)<code class="text-xs font-mono text-emerald-400">{{ $code }}</code>@endforeach
                        </div>
                    </div>
                    @endif
                ...
                @elseif($user->two_factor_secret && !$user->hasTwoFactorEnabled())
                    ...
                    <div class="inline-block border-4 border-white rounded-xl shadow-md mb-4 bg-white">{!! $qrCodeSvg !!}</div>
                    <form method="POST" action="{{ route('profile.two-factor.confirm') }}" class="flex gap-3 items-end">
                        ...
                        <div>
                            ...
                            <input type="text" name="code" maxlength="6" pattern="\d{6}" required class="form-input w-36 text-center text-lg tracking-widest font-mono {{ $errors->has('code') ? 'form-input-error' : '' }}" placeholder="000000"/>
                            @error('code')<p class="form-error">{{ $message }}</p>@enderror
                        </div>
                        <button type="submit" class="btn-primary btn-sm">Verify &amp; Enable</button>
                    </form>
                @else
                    <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">Two-factor authentication adds an extra layer of security. As a Senior Investigator, enabling 2FA is strongly recommended.</p>
                ...
                <div class="px-6 py-4 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-400">{{ $device->ip_address }} &bull; {{ $device->last_active_at?->diffForHumans() }}</p>
                        </div>
                    </div>
        ...
        <div class="card border-red-200 dark:border-red-900/50" x-data="{ show: false }">
            ...
            <div class="card-body">
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">Deleting your account will soft-delete it. An administrator can restore it if needed.</p>
